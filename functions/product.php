...
global $current_user;
  ...
  if ( !empty(doliconst("PRODUIT_MULTIPRICES")) && !empty($product->multiprices_ttc) ) {
    ...
    if ( doliCheckModules('discountprice') ) {
      ...
      $requestp = "/discountprice?productid=".$product->id."&sortfield=t.rowid&sortorder=ASC&sqlfilters=(t.date_begin%3A%3E%3D%3A'".$lastdate."')and(t.date_end%3A%3C%3D%3A'".$lastdate."')and(d.tosell:=:1)";
