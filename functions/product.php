...
$ln .= '<div class="float-end"><a type="button" type="submit" onclick="dolitest('.$line->fk_product.', 0, \'delete\');"><i class="fa-solid fa-trash-can"></i></a></div>';
$ln .= '</small></td></tr>';
}
...
$ln .= '<button type="button" class="btn btn-outline-secondary" type="submit" onclick="dolitest(0, 0, \'delete\');">'.__( 'Empty the basket', 'doliconnect').'</button>';
$ln .= '</div></div>';
return $ln;
} else {
...
if ( empty(doliconnectid('dolicart')) || empty(doliconnectid('dolicart')) ) {
  ...
  if ( $mstock['stock'] <= 0 || $mstock['m2'] < $mstock['step'] ) { 
    ...
    if ( $product->id == doliconst("ADHERENT_PRODUCT_ID_FOR_SUBSCRIPTIONS", dolidelay('constante')) ) {
      $button .= '<div class="d-grid gap-2">';
      if (empty($mstock['qty'])) {
        $button .= '<button class="btn btn-sm btn-warning mw-100" name="plus" type="submit">'.__( 'Add to basket', 'doliconnect').'</button><input id="qty-prod-'.$product->id.'" type="hidden" aria-label="Quantity" value="'.$mstock['qty'].'" readonly>';
      } else {
        $button .= '<button class="btn btn-sm btn-warning mw-100" name="delete" type="submit"><i class="fa-solid fa-trash-can"></i></button><input id="qty-prod-'.$product->id.'" type="hidden" aria-label="Quantity" value="'.$mstock['qty'].'" readonly>';
      }
      $button .= '</div>';
    } else {
      $button .= '<div class="input-group">';
      $button .= '<button class="btn btn-sm btn-warning" name="delete" type="submit" onclick="dolitest('.$product->id.', 0, \'delete\');"><i class="fa-solid fa-trash-can"></i></button>';
      $button .= '<button class="btn btn-sm btn-warning" name="minus" type="submit" onclick="dolitest('.$product->id.', 0, \'minus\');"><i class="fa-solid fa-minus"></i></button>
      ...
      <button class="btn btn-sm btn-warning" name="plus" type="submit" onclick="dolitest('.$product->id.', 0, \'plus\');"><i class="fa-solid fa-plus"></i></button>';
      if ( doliCheckModules('wishlist', $refresh) && !empty(get_option('doliconnectbeta')) ) {
        $button .= '<button class="btn btn-sm btn-light" name="wish" type="submit" onclick="dolitest('.$product->id.', 1, \'wish\');"><i class="fas fa-heart" style="color:Fuchsia"></i></button>';
      }    
      $button .= '</div>';
    }  
  }  
