...
global $current_user;
	if ( wp_verify_nonce( trim($_POST['dolicart-nonce']), 'dolicart-nonce') ) {
		...
		if ( isset($_POST['case']) && $_POST['case'] == "updateLine" && false === get_transient( $link ) ) {
			...
			$mstock = doliProductStock($product, true, true, isset($_POST['product-array'])?$_POST['product-array']:array(), isset($_POST['lineid'])?trim($_POST['lineid']):null);
			if ( isset($_POST['modify']) && $_POST['modify'] == "delete" ) { 
				...
				$result = doliaddtocart($product, $mstock, $qty, $price, isset($_POST['product-add-timestamp_start'])?trim($_POST['product-add-timestamp_start']):null, isset($_POST['product-add-timestamp_end'])?trim($_POST['product-add-timestamp_end']):null, null, isset($_POST['product-array'])?$_POST['product-array']:array());
				$newqty = $result['newqty'];
				if (doliCheckModules('relatedproducts') && !empty(doliRequiredRelatedProducts($product->id, null, false))) {
...
global $current_user;
	...
	if ( wp_verify_nonce( trim($_POST['dolimodal-nonce']), 'dolimodal-nonce' ) && isset($_POST['case']) && $_POST['case'] == "legacy" ) {
		...
		if ((!is_multisite() && get_option( 'users_can_register' )) || ((!is_multisite() && get_option( 'dolicustsupp_can_register' )) || ((get_option( 'dolicustsupp_can_register' ) || get_option('users_can_register') == '1') && (get_site_option( 'registration' ) == 'user' || get_site_option( 'registration' ) == 'all')))) {
			$modal['footer'] .=  "<a class='btn btn-lg btn-link fs-6 text-primary text-decoration-none col-6 m-0 rounded-0' href='".wp_registration_url(get_permalink())."' role='button' title='".__( 'Create an account', 'doliconnect')."'><small>".__( 'Create an account', 'doliconnect')."</small></a>";
		}
		if (get_site_option('doliconnect_mode')=='one') {
			restore_current_blog();
		}
		$modal['footer'] .= '<button class="btn btn-lg btn-link fs-6 text-primary text-decoration-none col-6 m-0 rounded-0 border-start" type="submit" value="submit"><strong>'.__( 'Sign in', 'doliconnect').'</strong></button>';
		$response['js'] = plugins_url( 'doliconnect/includes/custom/js/doligeneric.js');
	...
	/*} elseif ( wp_verify_nonce( trim($_POST['dolimodal-nonce']), 'dolimodal-nonce' ) && isset($_POST['case']) && $_POST['case'] == "renewmembership" ) {
		...
		*/
		...
		//$modal['footer'] = '<button name="case" class="btn btn-outline-secondary">'.__( 'Delete', 'doliconnect').'</button>';
		$modal['footer'] = '<button name="case" class="btn btn-outline-secondary" type="submit">'.__( 'Submit', 'doliconnect').'</button>';
