	...
	if ( wp_verify_nonce( trim($_POST['dolicart-nonce']), 'dolicart-nonce')) {
		if (isset($_POST['case']) && $_POST['case'] == "updateLine") {
			...
			if (isset($_POST['modify']) && $_POST['modify'] == "delete") { 
				...
				elseif (doliCheckRelatedProducts($product->id)) { $response['modal'] = doliModalTemplate('CartInfos', __( 'Related products', 'doliconnect'), 'to do', '<button type="button" class="btn btn-secondary" data-bs-dismiss="modal">'.__( "Close", "doliconnect").'</button>', 'modal-lg'); }
				wp_send_json_success($response);
				die();
