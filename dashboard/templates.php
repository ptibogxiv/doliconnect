...
function dolifaq_display($content) {
  ...
  if ( in_the_loop() && is_main_query() && is_page(doliconnectid('dolifaq')) && !empty(doliconnectid('dolifaq')) ) {
    ...
    if (dolicheckie($_SERVER['HTTP_USER_AGENT'])) {
      ...
      $request = "/knowledgemanagement/knowledgerecords?sortfield=t.rowid&sortorder=ASC&limit=".$limit."&page=".$page."&sqlfilters=(t.status%3D'1')%20and%20((t.lang%3D'0')or(t.lang%3D'".doliUserLang($current_user)."'))"; 
...
$requestp = "/products?sortfield=t.".$field."&sortorder=".$order."&category=".$categorie->id."&sqlfilters=(t.tosell%3A%3D%3A1)";
...
$requestp = "/products?sortfield=t.rowid&sortorder=DESC&category=".$categorie->id."&sqlfilters=(t.tosell%3A%3D%3A1)";
$listproduct = callDoliApi("GET", $requestp, null, dolidelay('product', esc_attr(isset($_GET["refresh"]) ? $_GET["refresh"] : null)));
if (empty($listproduct) || isset($listproduct->error)) {
$count = 0;
} else {
